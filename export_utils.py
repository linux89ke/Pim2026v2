...
def _normalize_export_df(df: pd.DataFrame, export_type: str) -> pd.DataFrame:
    df = df.copy()
    if export_type == 'simple':
        if 'ParentSKU' not in df.columns and 'PARENTSKU' in df.columns:
            df['ParentSKU'] = df['PARENTSKU']
        if 'SellerName' not in df.columns and 'SELLER_NAME' in df.columns:
            df['SellerName'] = df['SELLER_NAME']
    else:
        if 'SellerName' in df.columns and 'SELLER_NAME' not in df.columns:
            df['SELLER_NAME'] = df['SellerName']
    return df
...
def prepare_full_data_merged(data_df: pd.DataFrame, final_report_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if 'SellerName' in merged.columns and 'SELLER_NAME' not in merged.columns:
            merged['SELLER_NAME'] = merged['SellerName']
        for col in ["Status", "Reason", "Comment", "SellerName", "FLAG"]:
